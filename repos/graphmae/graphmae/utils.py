...
def build_args():
    ...
    parser.add_argument("--feat-pt", type=str, default=None, dest="feat_pt",
                        help=(
                            ...
                            "Expects a dict with key 'x' (Tensor [num_nodes, dim]). "
                            "When provided, graph.ndata['feat'] is replaced after load_dataset() "
                            "and before model construction. scale_feats() is NOT re-applied. "
                            "Default: None (use original dataset features)."
                        ))

    # encoder checkpoint export for eval/run_lp.py (Layer 2 Step 1 -> Step 2)
